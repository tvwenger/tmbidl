...
openu,lun,'/drang/grs_data/GRS_survey.dat',/get_lun,/append
;
...
;
kount=0
for il= nlong,1L,-1L do begin
    for ib = 1L,nlat do begin
        ...
        if ((il eq nlong) or (il eq 1)) and ((ib eq 1) or (ib eq nlat)) then begin
           print,l,b,l_idx,b_idx,xpix,ypix,lmap0,bmap0,delta_l,delta_b, $
                 l_ref,b_ref,v_cen,v_ref,delta_v,nchan,ll,bb,     $
                 format=fmt
           printf,lun,l,b,l_idx,b_idx,xpix,ypix,lmap0,bmap0,delta_l,delta_b, $
                 l_ref,b_ref,v_cen,v_ref,delta_v,nchan,ll,bb,     $
                 format=fmt
        end
        kount=kount+1
...
;
print
print,fname+' => '+mapfile+' kount= '+fstring(kount,'(i10)')
printf,lun,fname+' => '+mapfile+' kount= '+fstring(kount,'(i10)')
;
free_lun,lun
;
return
