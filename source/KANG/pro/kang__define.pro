...
IF N_Params() EQ 2 THEN BEGIN
    IF ~Keyword_Set(pixel) THEN $
      pix_xy = self.image->Convert_Coord(x, y, new_coords = 'Image') ELSE $
      pix_xy = [x, y]
ENDIF
...
; Transfer data into TMBIDL data format 
    ...
    l_gal=0.                    ;(lpix - lmap0)*delta_l   
    b_gal=0.                    ;(bpix - bmap0)*delta_b 
...
; Bring up dialog
    IF colorName EQ 1B THEN colorName = pickColorName(self.regions_cName, group_leader = self.tlbID);, filename = '~/idl/packages/kang/colors.txt')
...
valuelabelID = Widget_Label(toplabelsid, Value = 'Value')
self.valueID = CW_Field(toplabelsid, Title = '', Value = 0., /Float, /NoEdit, XSize = 14)
;valueID = Widget_Label(toplabelsid, Value = '  ', scr_xsize = 40)
;self.velLabelID = Widget_Label(toplabelsid, Value = 'Vel.')
;self.velValueID = CW_Field(toplabelsid, Title = '', Value = 0., /Float, /NoEdit, XSize = 14)
