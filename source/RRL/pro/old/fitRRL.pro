...
;   
;CALLING SEQUENCE: fitRRL, srcName, polType, lineType, CFLL=CFLL, CFRR=CFRR, nsaveOut=nsaveOut, outFile=outFile, fitDefault=fitDefault
;
;INPUTS:  
;     -srcName:    source name 
...
pro fitRRL, srcName, polType, lineType, CFLL=CFLL, CFRR=CFRR, nsaveOut=nsaveOut, outFile=outFile, fitDefault=fitDefault
...
for i=0,!nsave_max-1 do begin 
    if (!nsave_log[i] eq 1) then begin 
        ...
        if (polType eq 'L+R') then begin
            if (sname eq srcName) and (lineid eq lineType) then begin
